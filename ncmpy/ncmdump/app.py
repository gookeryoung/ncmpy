...
parser = argparse.ArgumentParser(prog="ncmdump", add_help=False)
parser.add_argument("-h", action="help", help="show this help message and exit")
parser.add_argument(
    "input", metavar="input", nargs="*", default=["."], help="ncm file or folder path"
)
parser.add_argument(
    "-f", metavar="format", dest="format", default="", help="customize naming format"
)
parser.add_argument(
    "-o", metavar="output", dest="output", help="customize saving folder"
)
parser.add_argument(
    "-d", dest="delete", action="store_true", help="delete source after conversion"
)
group = parser.add_mutually_exclusive_group()
group.add_argument(
    "-c", dest="cover", action="store_true", help="overwrite file with the same name"
)
group.add_argument(
    "-r", dest="rename", action="store_true", help="auto rename if file name conflicts"
)
...
def traverse(path):
    ...
    if not os.path.exists(path):
        ...
        return sum(
            [traverse(os.path.join(path, name)) for name in os.listdir(path)], []
        )
    else:
...
def main():
    ...
    for path in files:
        try:
            save = ncmdump(path, name_format, not args.cover)
            if save:
