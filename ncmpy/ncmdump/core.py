...
def ncmdump(input_path: str, output_path: str = "", skip=True):
    output_path = (
        (lambda path, meta: os.path.splitext(path)[0] + "." + meta["format"])
        if not output_path
        else output_path
    )
    output_path_generator = (
        (lambda path, meta: output_path) if not callable(output_path) else output_path
    )
    ...
    if meta_length:
        ...
        # TODO: FiXME
        raw_data = '{"' + f"{meta_data[6:]}"
        print(raw_data, type(raw_data))
        meta_data = json.loads(raw_data)
    else:
        meta_data = {
            "format": "flac" if os.fstat(f.fileno()).st_size > 1024**2 * 16 else "mp3"
        }
    ...
    def embed(item, content, type):
        ...
        item.mime = (
            "image/png"
            if content[0:4] == binascii.a2b_hex("89504E47")
            else "image/jpeg"
        )
        item.data = content
